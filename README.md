...
Develop a Predictive Model: Build a classifier model that achieves: At least 90% accuracy in predicting customer churn. A minimum precision of 70%, ensuring the model minimizes false positives and provides reliable predictions.
...
## CONCLUSIONS
Through data analysis and model building, we identified key features such as 'account length', 'customer service calls', 'churn', 'international plan_yes', 'voice mail plan_yes', 'total calls', and 'total charge' that significantly predict customer churn. The Decision Tree Classifier performed well, while the Logistic Regression model struggled despite SMOTE and MinMax scaling. The Decision Tree performed well and was the highly effective one.
...
Fine-tuning the  Decision Tree model along with experimenting with alternative sampling techniques, can help improve prediction accuracy, especially in identifying at-risk customers.
